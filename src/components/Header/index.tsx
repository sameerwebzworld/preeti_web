...
const Header = () => {
  ...
  // search icon (reused in desktop + mobile search fields)
  const searchIcon = (
    <svg
  ...
  // main nav list (shared between desktop + mobile)
  const navItems = menuData.map((menuItem, i) =>
    menuItem.submenu ? (
  ...
  return (
    ...
    >
      ...
      <div className="max-w-[1170px] mx-auto px-4 sm:px-7.5 xl:px-0">
        ...
        >
          ...
          <Link className="flex-shrink-0" href="/">
            <Image
              ...
              width={52}
              height={52}
              className="h-11 w-11 sm:h-12.5 sm:w-12.5 object-contain"
              priority
            />
          </Link>
          ...
          <nav className="hidden xl:flex flex-1 justify-center">
            <ul className="flex items-center gap-8">{navItems}</ul>
          </nav>

          {/* <!-- Right: search + icons --> */}
          <div className="flex items-center gap-4 sm:gap-5">
            {/* <!-- Search (tablet/desktop, beside caller) --> */}
            <form
              onSubmit={(e) => e.preventDefault()}
              className="hidden md:block"
            >
              <div className="flex items-center rounded-md border border-gray-3 bg-gray-1 ease-out duration-200 focus-within:border-blue">
                <input
                  ...
                  autoComplete="off"
                  className="custom-search w-36 lg:w-52 bg-transparent py-2 pl-4 pr-1 text-custom-sm text-dark outline-none"
                />
                <button
                  type="submit"
                  aria-label="Search"
                  className="flex items-center justify-center px-3 py-2 text-dark ease-in duration-200 hover:text-blue"
                >
                  {searchIcon}
                </button>
              </div>
            ...
            <Link
              ...
              className="hidden sm:flex items-center hover:opacity-70 ease-out duration-200"
            >
              <svg
                width="24"
                height="24"
            ...
            <Link
              href="/signin"
              aria-label="Account"
              className="flex items-center hover:opacity-70 ease-out duration-200"
            >
              <svg
                width="24"
                height="24"
        ...
        >
          <div className="flex items-center rounded-md border border-gray-3 bg-gray-1 focus-within:border-blue ease-out duration-200">
            <input
              onChange={(e) => setSearchQuery(e.target.value)}
              value={searchQuery}
              type="search"
              ...
              placeholder="Search products..."
              autoComplete="off"
              className="custom-search w-full bg-transparent py-2.5 pl-4 pr-1 text-custom-sm text-dark outline-none"
            />
            <button
              type="submit"
              aria-label="Search"
              className="flex items-center justify-center px-3.5 py-2.5 text-dark hover:text-blue"
            >
              {searchIcon}
            </button>
          </div>
        ...
        {/* <!-- Mobile Nav dropdown --> */}
        <nav
          className={`xl:hidden overflow-hidden ease-in-out duration-300 ${
            navigationOpen
              ? "max-h-[420px] overflow-y-auto border-t border-gray-3 py-4"
              : "max-h-0"
          }`}
        >
